...
WEAPON_MOVESETS = [
    ...
    ("warmace", "vanilla_mace", "fist",
     "WarmaceItem's hurtEnemy/postHurtEnemy delegate straight to vanilla's MaceItem, so Better Combat's vanilla_mace preset (built for that exact vanilla item) is an exact behavioral match; Epic Fight has no mace type, so fist (its own blunt-impact analog) is the closest of its 17"),
    ("battlesign", "staff", "shield",
     "a flat implement on a handle whose innate blocks and reflects: Epic Fight's shield type is a direct thematic match, Better Combat has no shield-shaped preset so staff (its closest pole-mounted guard implement) stands in"),
    ("frying_pan", "hammer", "fist",
     "a single blunt head on a handle swings the same short crushing arc as both mods' blunt-impact presets"),
    # -- Axe family --
