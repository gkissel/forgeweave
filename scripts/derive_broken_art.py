...
Five tools have no upstream counterpart at all -- the dagger, katana and scimitar (issue #159/#198),
the vein hammer (#157) and the war mace (#161) -- so there is no broken art to port for them. Their
broken layers are instead an algorithmic transform of their own already-derived head layer, the same
"derived pixels through a documented transform" treatment `generate_pattern_textures.py` and
`generate_cast_textures.py` give their outputs (NOTICE.md carries a row per file either way, since
every input pixel is still upstream's).
...
def main() -> None:
    ...
    for name in CHIPPED:
        ...
        if not intact.is_file():
            raise SystemExit(f"expected the already-derived {intact} to exist")
