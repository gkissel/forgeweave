...
public class ForgeweaveItemModelProvider extends ItemModelProvider {
    ...
    protected void registerModels() {
        ...
        // texture is a straight upstream port under derived/item/ except vein_hammer_head, which has
        // no 1.12/1.20 counterpart and lives under the plain item/ folder instead (NOTICE.md).
        ...
        singleLayerModel(ForgeweaveItems.PART_VEIN_HAMMER_HEAD, itemTexture("vein_hammer_head"));
        // #161: derived from the clone's hammer head with a minimal reshape (issue #198's decision:
        // no freshly-authored art -- scripts/derive_warmace_art.py, NOTICE.md), so unlike the vein
        // hammer head above it lives under derived/item/ with the rest of the ported parts.
        singleLayerModel(ForgeweaveItems.PART_WAR_MACE_HEAD, derivedItem("war_mace_head"));
        // #159's curved blade is the second part with no upstream counterpart, so like the vein
        // hammer head its art is freshly authored and lives outside derived/ (NOTICE.md has no row).
        singleLayerModel(ForgeweaveItems.PART_CURVED_BLADE, itemTexture("curved_blade"));
        // #160's katana blade, same standing: a Forgeweave-own shape with no upstream part to derive.
