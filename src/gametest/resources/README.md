...
| `data/forgeweave/forgeweave/material/gametest_writable.json` | A paper-shaped material carrying `writable` (general) + `writable2` (head), so `MiningTraitGameTests` can assemble an all-paper-like tool through a real Tool Station and assert the +2 free-slot math (issue #228). The real paper material lands with the M3.2 roster batches. |
| `data/forgeweave/forgeweave/material/gametest_squeaky.json` | A sponge-shaped material carrying `squeaky`, so `MiningTraitGameTests` can assert the assembly-time Silk Touch grant and the hard-zero attack damage on a station-assembled tool (issue #228). The real sponge material lands with the M3.2 roster batches. |
