...
| `~/development/minecraft/references/tinkers-1.20` | `1.20.1` | `b98c7867f7fcb66d3f60a90d30deeeae26ec032f` | Modern API idioms only: registries, data components, networking, datagen, GameTests on recent Forge — adapt patterns, not features. |
...
| `~/development/minecraft/references/tool-leveling-1.12` | `master` | `5d7a2c96801fc539397e94bd9da671aa293b484e` | **Feature target for M7.** [Tinkers' Tool Leveling](https://github.com/SlimeKnights/TinkerToolLeveling) by boni/SlimeKnights, **MIT — direct port allowed**, `NOTICE.md` rows per derived file (code and the `sounds/chime.ogg` asset alike), same rules as the Tinkers' clones. Tools gain XP from intended use; each level grants a modifier slot. Note it has **no armor code** — M7's armor leveling is original Forgeweave design, not derived from here (docs/SCOPE.md § Milestone 7, D-M7-2). |
| `~/development/minecraft/references/spartan-weaponry-1.12.2` | `1.12.2` | `af87ea162cc043f1ea4236e5da5e723c600001ed` | **Not Tinkers'. Historical only.** [Spartan Weaponry](https://github.com/ObliviousSpartan/SpartanWeaponry), **Apache-2.0, not MIT.** Issue #375 derived the katana and scimitar blade art from it; the 16px Forged batch (2026-09-05) replaced every one of those files with original art, so nothing ships from this clone any more and nothing new may derive from it. `licenses/APACHE-2.0-SpartanWeaponry.txt` stays as the record. |
...
- The 1.20 pin moved on 2026-09-18 from `de26560d26c15edf93e6078520202d1c0518394e` to the commit in the table, to pick up upstream's armor stand preview and current Modifier Worktable. `NOTICE.md` rows written before that date keep the old hash: a row names the commit its file was derived from, which is not always the current pin.
...
- No shipped file derives from the Spartan Weaponry clone any more (see its row). Deriving from it again would need a maintainer decision, a `NOTICE.md` row *and* an entry in the modification notice in `licenses/APACHE-2.0-SpartanWeaponry.txt`, because Apache-2.0 unlike MIT requires shipping the license text and marking modified files.
...
- If the clones are missing, re-create them: `git clone --depth 1 --branch <branch> https://github.com/SlimeKnights/TinkersConstruct.git <path>` (or `.../SlimeKnights/Mantle.git` for the Mantle row, `.../SlimeKnights/TinkerToolLeveling.git` for the tool-leveling row) and checkout the pinned commit.
...
**Two constraints on the art sets (maintainer directive, 2026-09-05).** The sprite standard is **16x16** and is not changing: no 32px renders, whatever the designer's source resolution. And the Legacy pack is **Tinkers'-native art only**, from either clone: it may override a file only where the pre-Forged pixels came from Tinkers' Construct itself. Forgeweave-only tools and parts (the katana, scimitar and warmace, plus `curved_blade`/`war_mace_head`) always use Forgeweave's own art and never appear in the Legacy pack; only their generated composites (pattern, cast, clay cast) do, because those recompose over the Legacy pack's own Tinkers'-native blank. `LegacyResourcePackTest#legacyShipsNothingForForgeweaveOnlyTools` enforces the second rule.
