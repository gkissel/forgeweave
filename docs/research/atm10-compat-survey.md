...
7. **Silent Gear's tool system stays skipped; its metals shipped as presets anyway.** Corrected 2026-09-18 (issue #1058, D-M8-24, maintainer decision): five Silent Gear metals (crimson steel, azure silver, azure electrum, blaze gold, tyrian steel) are now Track A material presets, since they are real, MIT-licensed ingots the same as any other compat metal. What stays skipped is Silent Gear's own tool-and-armor progression system -- Forgeweave never talks to it. Extreme Reactors and XyCraft stay skipped outright: reactor fuel that is not tool-worthy, and a low-confidence legacy mod.
...
- **Silent Gear, Silent's Gems, Silent Gear Metalworks' tool system.** A second material-based tool-and-armor mod in the same pack as Forgeweave is not a compat target, it is a competitor for the same player attention. Nothing about Forgeweave needs to talk to it, and building a bridge between two full tool-progression systems is a design problem with no clear win. Corrected 2026-09-18 (issue #1058, D-M8-24): this applies only to the tool system. The base mod's five endgame metals shipped as ordinary Track A material presets, the same treatment any other MIT-licensed compat metal gets.
