...
| Draconic Evolution | 4 | Draconium, Wyvern Metal, Draconic Metal, Chaotic Metal — energy-powered, each a tier upgrade of the last via fusion crafting, innate soul-steal. **Forgeweave's answer (M8, [#946](https://github.com/gkissel/forgeweave/issues/946)):** the four tag/core-gated preset materials (`draconium`, `wyvern`, `draconium_awakened`, `chaotic`) stay as the raw tier, and three Forgeweave metals — emberweld, starweld, voidweld — sit above them, each made only by a fusion craft on DE's own multiblock and each carrying `evolved` I–III plus `soulrend` I–III. Own ids, own stats, own recipes; nothing derived from this reference |
