...
| Draconic Evolution | 4 | Draconium, Wyvern Metal, Draconic Metal, Chaotic Metal — energy-powered, each a tier upgrade of the last via fusion crafting, innate soul-steal. **Forgeweave's answer (M8, [#946](https://github.com/gkissel/forgeweave/issues/946)):** the tag/core-gated preset materials stay as the raw tier, and three Forgeweave metals — emberweld, starweld, voidweld — sit above them, each made only by a fusion craft on DE's own multiblock and each carrying `evolved` I–III plus `soulrend` I–III. Own ids, own stats, own recipes; nothing derived from this reference. **Roster split ([#953](https://github.com/gkissel/forgeweave/issues/953), maintainer directive 2026-09-04):** the presets are now five, in two kinds. Two ingot metals -- `draconium` (blue-violet `#5B4BC4`, `surging`) and `draconium_awakened` (orange `#F06000`, `surging2`) -- keep the smeltery, cast and melt as before. Three core materials -- `wyvern` (`stonewake`), `awakened` (new, `ruthless`) and `chaotic` (`chaosmark`) -- are Part Builder only: no molten form, no melting or casting recipe, no `cast_only` flag. Each core also carries `evolved` I/II/III, so a core-built tool is fusion upgradable up to its own tier without going through a fusion metal first. **Fourth tier ([#965](https://github.com/gkissel/forgeweave/issues/965), maintainer directive 2026-09-04):** Draconic Evolution has four `TechLevel`s, not three, so Forgeweave now follows all four. A sixth preset, `draconium_core` (slate `#3E4A63`, `coremend`), sits under the three cores, and a fourth fusion metal, `duskweld` (plum `#7E2A6E`, `soulwick`), sits under emberweld at DE's inert tier. The tier marker below `evolved` is a new id, `evolving`, so the three shipped ids keep the tiers they already meant and a tool in a save does not move |
