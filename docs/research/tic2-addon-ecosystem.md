...
**Re-verified 2026-08-31 — verdict unchanged.** Local read-only clone of the MinecraftModDevelopmentMods fork (`~/development/minecraft/references/moar-tinkers-1.12` @ `afdb44a`) confirms there is no license file anywhere in the tree *or its git history* — the fork stripped even the MDK boilerplate (its HEAD commit is literally "Delete CREDITS-fml.txt"), `mcmod.info` has no license field (`authorList: ["Bartz24"]`, credits YaibaToKen "for a rewrite"), and no Java file carries a copyright header. GitHub API cross-check: `Bartz24/MoarTinkers` license = `Other`/`NOASSERTION` (the MDK `LICENSE-new.txt` is what GitHub fails to classify); the MMD fork and the YaibaToKen redirect report no license at all. Not listed on Modrinth. The GPLv3 claim exists only as CurseForge page metadata with no primary-source file behind it — and even if a GPLv3 grant surfaced, GPLv3 is copyleft and ADR-0003 admits only MIT for derivation, so this stays inspiration-only either way.
