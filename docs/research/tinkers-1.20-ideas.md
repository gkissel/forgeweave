...
- **Slime islands, geodes, and other world content**: explicit non-goal until the world-content milestone is scoped (`docs/SCOPE.md:75`, `docs/SCOPE.md:247`). Nothing here changes that.
...
|---|---|---|---|---|---|
| Armor stand preview | Preview a tool/armor build before committing | Not independently re-confirmed in this pass (out of survey scope) | Forgeweave already has this per the maintainer's own framing in the issue body ("the way the Modifier Worktable and the armor stand preview were picked") | n/a | Already shipped |
...
| Tool renaming | Rename a tool at the crafting station | Confirmed as an Anvil-tier perk: "let you rename tools during any modification" (`book/mighty_smelting/en_us/intro/tinkers_anvil.json`) | Not confirmed present in Forgeweave's Tool Station/Tool Forge | S | Small, self-contained UI add; low risk |
...
Most of the "quality of life" items the issue asked about turn out to already be shipped in Forgeweave, several explicitly cited by the maintainer's own issue framing (armor stand preview) or by SCOPE.md (JEI transfer, guide book, leveling tooltip, Tool Station panel). The two genuinely open items are **tool renaming** and, more speculatively, **held-key tooltip detail** if a concrete use case shows up.
