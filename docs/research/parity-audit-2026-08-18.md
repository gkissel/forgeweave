...
| Slime islands (shell, lakes, trees, plants) | `SlimeIslandGenerator:51-320` | biomes/structures | `worldgen/SlimeIslandStructure`+`SlimeIslandPiece`+`SlimeIslandShape` (#449, structure since #629 so `/locate` finds them): shell, grass, trees, plants; no lake, no vines | partial | Y (SCOPE:75, #449) | medium | T18 |
