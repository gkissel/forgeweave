...
| Page chrome: hover tooltips, links, back arrow | Mantle elements + `ContentMaterial` tooltips | n/a | arrows + page numbers + index links + listing/grid links with hover styling and icon hover names (#479) | partial | Y (PR #360, #479) | low | #633 |
...
| Material page (side column, part icons, hover desc, flavour) | `ContentMaterial:62-215` | n/a | title + flat stat lines + trait names | partial | Y (PR #360) | medium | #633 |
...
- [ ] **T48 — Book content parity pass** (partially shipped) — the navigation spine shipped in **#479**: upstream's `ContentListingSectionTransformer` listing page at index 0 of the tools and modifiers sections (rows labelled from the registered tool/modifier, not the page title, per `ToolSectionTransformer`/`ModifierSectionTransformer`), upstream's `AbstractMaterialSectionTransformer` icon-grid overview in front of the materials section, and the link chrome both need (`ElementListingLeft`'s `"- "` → `" > "` dark-red hover, `ElementPageIconLink`'s hover wash and coloured-name tooltip). **Audit correction:** the row above claimed hover tooltips and links were "partial (PR #360)" — before #479 the book had *no* hover tooltip and no link other than the index rows. The rest of the entry — the material/tool/modifier page layouts, the fortify variant, the bow-materials section and the smeltery 3D structure page — is **#633**; the resource-pack JSON structure stays gated on the engine port, **#430** (book, medium).
