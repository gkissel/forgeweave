...
| slimey green / blue | `TraitSlimey:38-58` (blue slime entity) | n/a | green spawns vanilla Slime, blue spawns `forgeweave:blue_slime` (#451) | have | Y (PR #241, #451) | none | — |
...
| Shovel/excavator grass paths | `Shovel.java:64-97` | ItemAbility SHOVEL_FLATTEN | `ShovelPath` via `ToolItem#useOn` (#464) | have | — | none | — |
...
| Frypan boing sound / Bane of Pigs | `FryPan.java:51-132` | n/a | both boings present (`HeavyKnockback` on a landed blow at 2.0/1.0, `ChargedLaunch` on the launch at 1.5 / `0.6 + 0.2 * random`), vanilla stand-in sound event; no Bane of Pigs | partial | Y for the cue's sound event (#481, decision deferred to #566) | low | T50 (Bane of Pigs only) |
...
| Recipe-derived melting of metal-crafted items (+ rails, horse armor) | `TinkerSmeltery:392-399,746-873` | explicit rows | 17 `{iron,gold}_*` melting rows | have | Y (#439, the scan run once at authoring time) | none | T7 |
...
| Tinker tank multiblock | `TileTinkerTank`, `MultiblockTinkerTank` | dropped | `SearedReservoir*` (renamed per CONTEXT.md) | have | Y (#475) | none | — |
...
| Output take: consume, events, craft sound | `ContainerToolStation:177-214` | n/a | `ToolStationMenu:542-613` + the craft cue on every take (Station 0.8 / `0.8 + 0.4 * random`, Forge vanilla `BLOCK_ANVIL_USE` 0.9 / `0.95 + 0.2 * random`) | have | Y for the Station cue's sound event (#481, decision deferred to #566) | none | — |
...
| Book cover (title, subtitle/author, cover colour) | `appearance.json` | n/a | `appearance.json`'s own `coverColor` 0xffce85 and `GuiBook`'s title/subtitle scaling (#430); subtitle reworded | have | Y (#430; upstream's subtitle is its author's credit, which Forgeweave cannot reuse as its own book's) | none | — |
| Index page with item icons | `book/index.json` | n/a | `BookContent:72-115` | have | Y (PR #360) | none | — |
| Page chrome: hover tooltips, links, back arrow | Mantle elements + `ContentMaterial` tooltips | n/a | arrows + page numbers + index links + listing/grid links with hover styling and icon hover names (#479) + per-line and per-icon hover text on material pages (#633) | have | Y (PR #360, #479, #633) | none | — |
| Page text overflow | Mantle element boxes | n/a | `BookLayout` paginates + scissor clip | have | Y (#428 closed, `53df05c`) | low | — |
| Per-section listing pages | `ContentListingSectionTransformer:20-45` | n/a | `BookPage.ListingPage` at index 0 of tools and modifiers (#479) | have | Y (#479) | none | — |
| Materials overview icon grid with links | `AbstractMaterialSectionTransformer:36-66` | n/a | `BookPage.IconGridPage` opens the materials section (#479) | have | Y (#479) | none | — |
| Material page (side column, part icons, hover desc, flavour) | `ContentMaterial:62-215` | n/a | display bar (representative item, Part Builder/Casting Basin, demo tools), per-stat-type blocks with cycling part icon + underlined name, hover text on every stat and trait line, flavour quote (#633) | have | Y (#633) | none | — |
| Bow materials section | `BowMaterialSectionTransformer` | n/a | bow stats folded into material pages | partial | Y (#403/#412) | low | #633 |
| Tool page (properties list, station slot graphic, parts) | `ContentTool:83-146` | n/a | icon + name + station blurb | partial | Y (PR #360) | medium | #633 |
...
| Modifier page (colour title, effects list, demo tools, inputs) | `ContentModifier:96-192` | n/a | name + description, alphabetical | partial | Y (PR #360) | medium | #633 |
| Fortify page variant | `ContentModifierFortify:14-40` | n/a | absent | missing | N | low | #633 |
| Smeltery section (image, structure text, 3D structure page) | `book/sections/smeltery.json` | n/a | intro + structure + FW page; no 3D | partial | Y (PR #360) | low | #633 |
...
| Book data as resource-pack JSON + per-locale folders | `assets/tconstruct/book/**` | n/a | hard-coded `BookContent` + lang keys | partial | Y (PR #360) | low | #430 |
...
| Custom sounds (saw, anvil, frypan boing, squeak, shocking) | `Sounds.java:27-38` (CC-BY/CC0) | n/a | every cue plays, none of the assets ship: vanilla stand-ins for saw/frypan/squeak/shocking/crossbow, upstream's own vanilla anvil at the Forge | partial | Y (#415 shocking, #495 squeaky, #481 saw + frypan) | medium | #566 |
...
| Slime islands (shell, lakes, trees, plants) | `SlimeIslandGenerator:51-320` | biomes/structures | `worldgen/SlimeIslandStructure`+`SlimeIslandPiece`+`SlimeIslandShape` (#449, structure since #629 so `/locate` finds them; vines since #488): shell, grass, trees, plants, vines; no lake | partial | Y (SCOPE:75, #449/#488) | low | T18 |
| Magma slime islands (Nether) | `MagmaSlimeIslandGenerator:30-79` | n/a | absent | missing | Y (SCOPE:75) | high | T19 |
| Blue slime entity + island spawns | `EntityBlueSlime`, `WorldEvents:29-33` | sky/ender slimes | `ForgeweaveEntities#BLUE_SLIME` + `BlueSlimeRenderer` + the island structure's `spawn_overrides` (#451); vanilla slime ball stands in for the blue one | have | Y (SCOPE:245, #451) | low | T57 |
...
| Slime sapling → slime tree | `SlimeTreeGenerator` | n/a | `SlimeSaplingBlock` + `SlimeIslandShape#plantTree` (#488); dropped by slime leaves at upstream's 1/25 | have | Y (#488) | none | — |
| Slime vines (+ bowstring material) | `BlockSlimeVine`, `TinkerWorld:77-102` | n/a | `SlimeVineBlock` x3 stages x2 colours + `slimevine_blue`/`slimevine_purple` bowstrings (#488) | have | Y (#488) | none | — |
| Congealed slime blocks (5 colours) | `BlockSlimeCongealed` | n/a | all six colours (#449 green, #450 magma, #625 blue/purple, #635 blood/pink), each crafted 4 balls <-> 1 block since #635 | have | Y (#635) | none | — |
| Colored slime blocks + matchVanillaSlimeblock | `BlockSlime:19-68` | n/a | `ColouredSlimeBlock` x5 + `MixedSlimeBlockRecipe` behind `matchVanillaSlimeblock` (#635); added alongside vanilla's slime block recipe rather than replacing it | have | Y (#635) | none | — |
| Colored slime balls (edible) + slime drops | `TinkerCommons:140-144,293-297` | n/a | the five edible slime balls at upstream's nutrition/saturation/effects (#635); the four *slime drops* (`TinkerCommons:293-297`, a Gadgets-pulse food) are still absent | partial | Y (#635) | low | T57 |
| Blue + purple slime fluids | `TinkerFluids:60-61,194-236` | 4 slime fluids | `blue_slime`/`purple_slime` (#625); purple's melting recipes and the knightslime alloy's purple input since #635 | have | Y (#625/#635) | none | — |
| Purple slime → knightslime alloy chain | `TinkerSmeltery:369-374,536-540` | n/a | purple slime melting (ball/congealed/block) + the alloy's 125 mB purple input (#635); #232's green substitute is gone | have | Y (#635) | none | — |
...
| Blood slime ball casting | `TinkerSmeltery:364-367` | n/a | 160 mB of blood on a casting table (#635) | have | Y (#635) | none | — |
| Slimy mud → slime crystals | `BlockSoil` SLIMY_MUD ×3 | n/a | all three muds at upstream's own recipes and smelts (#635 added blue and put the magma slime balls back into the magma mud) | have | Y (#635) | none | — |
...
| Guide book bow pages + Bow Materials section | `book/en_us/tools/*`, `index.json:33-37` | n/a | bows in tool list; stats on material pages | partial | Y (PR #412) | low | #633 |
...
| Tool-class exposure (vanilla tool tags, ItemAbilities) | Forge tool classes | 1.20 tool tags + actions | five vanilla tool tags + six `c:tools/*`; `ToolItem#abilitiesFor` per kind (#464) | have | — | none | — |
...
- [x] **T7 — Melt vanilla metal-crafted items** (shipped, #439) — 17 melting rows covering iron/gold tools, armor, buckets, rails, horse armor, minecarts and the rest of 1.12's metal-only crafting recipes. Upstream's ignore list (`Config.oredictMeltingIgnore`) and its crafting-registry scan are inputs to a *runtime* derivation; the scan was run once against 1.12 at authoring time and its results shipped as datapack rows instead, so there is nothing left at runtime for an ignore list to filter — the same move upstream itself made on the 1.20 branch. Post-1.12 metal items (chain, lantern, crossbow, chainmail, blast furnace, stonecutter, smithing table, bell) and durability-scaled melting are follow-up (#583) (smeltery, high, M6/backlog).
...
- [ ] **T18 — Slime islands** — first slice shipped (#449): the island feature itself (elliptical plug, both erosion passes, grass surfacing, the palette roll, upstream's plant scatter and its congealed-slime trees), the slime dirt, grass, congealed slime, leaves and tall grass/fern blocks with upstream's colour-map foliage tinting, and all five `slimeIsland*` configs. Converted from a placed feature to a **structure** in #629 (`SlimeIslandStructure`/`SlimeIslandPiece`, structure set + biome tag), following upstream's own 1.20 branch, so `/locate structure forgeweave:slime_island` works. The slime **vines** shipped with #488, which turned upstream's `vine == null` canopy branch back on. Still open: the slime **lake** (blocked on the blue/purple slime fluids, T57), with upstream's own `lakeGenerator == null` branch standing in meanwhile (world, high, world-content milestone).
- [ ] **T19 — Magma slime islands** — Nether island feature plus the magma-cube spawn override (world, high, world-content milestone).
- [x] **T20 — Blue slime entity** (shipped, #451) — `forgeweave:blue_slime` ports `EntityBlueSlime` whole: vanilla slime's own body, upstream's greyscale slime texture tinted `0xff67f0f5` on both the body and the gel layer, upstream's one-pool 0-2 drop with its 0-1 looting bonus gated to size 1, the slime-grass-only spawn rule, and -- as the island structure's `spawn_overrides` (#629 made the island a structure, which is what makes that reachable) -- upstream's own clear-the-monster-list plus its one weight-15/2-4 entry, so an island really does spawn blue slimes and nothing else. Plus the `0x47eff5`/`0xacfff4` spawn egg. `slimeyBlue` now spawns it, closing that trait's own stand-in. No entity class of its own: 1.21 answers upstream's four overrides elsewhere (`Slime#remove` splits off `getType()`, a loot-table size condition replaces `getLootTable`, the table names the item `dropItemWithOffset` forced, and `SpawnPlacements` carries `getCanSpawnHere` -- which is where upstream's own 1.20 branch put it). Two recorded reductions, both waiting on T57's coloured slimes: the drop is the vanilla slime ball rather than the blue one, and the death particle is vanilla's `item_slime` (which *is* the slime ball) for the same reason. The liquid-slime spawn branch waits on T57's fluids (world, high, world-content milestone).
- [ ] **T21 — Slime boots** — bounce handler and fall-damage cancel (world, high, M5 / #26).
- [x] **T22 — Slime sling** (shipped, #453) — `SlimeSlingItem` ports `ItemSlimeSling` whole: the bow charge curve times four clamped at 6, the on-ground and block-target gates, the fling along the inverted look vector at a third force vertically, upstream's own `slimesling.ogg` and item art, and `SlimeBounceHandler`'s airborne momentum carry (its bounce arm waits on T21's boots). Upstream's hand-rolled `EntityMovementChangePacket` needs no counterpart: `hurtMarked` makes vanilla send the player its own `ClientboundSetEntityMotionPacket`. Two recorded reductions -- one sling instead of upstream's five colours, and a vanilla slime block standing in for congealed slime in the recipe. The second was reverted by #635, which gave the sling (and the boots) real green congealed slime; the one-sling reduction stands.
...
- [x] **T30 — Repair accepts only the plain repair item** (shipped, #461) — repair now matches the same item table the Part Builder does (upstream's one `Material extends RecipeMatchRegistry`): every `crafting_items` entry at its own shard-unit value, plus the material's shard, plus the material's `repair_item` kept as a one-ingot floor for the few tags wider than the crafting-item list. Nuggets stay out until T58 gives the schema a finer unit than the shard, and storage blocks for cobalt/ardite/manyullyn until T61 lists them (materials/stations, medium, M6).
...
- [x] **T33 — Tool tags, ItemAbilities and grass paths** (shipped, #576) — every tool into the vanilla tag for its upstream tool class (`minecraft:pickaxes/shovels/axes/hoes/swords`, which is also all NeoForge's `c:tools` reads) plus `c:tools/{melee_weapon,mining_tool,shear,mace,bow,crossbow,ranged_weapon}`; `ToolItem#abilitiesFor` derives the `canPerformAction` set from the tool's own `mineable/*` tags, Broken refusing everything; and `ShovelPath` ports `Shovel#onItemUse`'s grass paths over the shovel's one block and the excavator's 3x3. Three recorded deviations: the sword set omits `SWORD_SWEEP` (upstream's `SwordCore` never got vanilla's sweep and `BroadswordSweep` already ports the by-hand one, so claiming it would sweep twice), each kind claims only its `_DIG` ability plus what Forgeweave actually implements (upstream 1.20's own `ToolActionsModule.of(...)`-plus-traits split; stripping/scraping/wax removal are #575), and `ToolItem#isBookEnchantable` now declines the anvil's book merge the new `#minecraft:enchantable/*` membership would otherwise open, on the existing `allowVanillaEnchanting` toggle (tools/config, medium, M3 playtest-fix).
...
- [x] **T43 — Bucket-filling casting recipe** (shipped, #542; re-shaped by #604) — one `casting_recipe/bucket.json` row with no `fluid` field, matching whatever is poured and filling the cast bucket through the item's own fluid capability, exactly as upstream's `BucketCastingRecipe` does. #542 first shipped it as one `bucket_<fluid>.json` row per fluid this mod registers (issue #286); "same player-facing result" was wrong — water, which melting ice and snow produces, is a vanilla fluid with no molten-metal row, so it could not be bucketed off a casting table at all (playtest alpha.3 item 34.a).
- [x] **T44 — Tinker tank multiblock** (shipped, #475) — the **Seared Reservoir**: upstream's `MultiblockTinkerTank` rules (floor of seared blocks/glass/drains, walls and corners taking the full `validSmelteryBlocks` set, ceiling of any wall block or a bottom-half seared slab/stairs, and — unlike the smeltery and the furnace — **no tank requirement**), `TileTinkerTank`'s shell-inclusive `(w+2)(h+2)(d+2) * 4000 mB` capacity, `GuiTinkerTank`'s 122x130 slotless panel with the 106x106 fluid column, and the drain generalised to pour whichever multiblock claimed it (upstream's `ISmelteryTankHandler`). Named "Seared Reservoir" rather than "Tinker Tank" because CONTEXT.md's avoided terminology rules the word out; that rename is the ticket's only deviation. The guide book's tinker-tank section stays open under the book row above (smeltery, medium, M6/backlog).
...
- [ ] **T48 — Book content parity pass** (partially shipped) — the navigation spine shipped in **#479**: upstream's `ContentListingSectionTransformer` listing page at index 0 of the tools and modifiers sections (rows labelled from the registered tool/modifier, not the page title, per `ToolSectionTransformer`/`ModifierSectionTransformer`), upstream's `AbstractMaterialSectionTransformer` icon-grid overview in front of the materials section, and the link chrome both need (`ElementListingLeft`'s `"- "` → `" > "` dark-red hover, `ElementPageIconLink`'s hover wash and coloured-name tooltip). **Audit correction:** the row above claimed hover tooltips and links were "partial (PR #360)" — before #479 the book had *no* hover tooltip and no link other than the index rows. The **material page layout** shipped next (#633's first slice): upstream's `ContentMaterial#addDisplayItems` bar (representative item, Part Builder/Casting Basin icons, demo tools built wholly from the material, nine icons in all), `#addStatsDisplay`'s per-stat-type blocks (the parts that read the stat as a cycling half-scale icon beside the underlined stat-type name), `#getStatLines`/`#getTraitLines`' hover text on every stat and trait line, and the italic `<material>.flavour` quote. Still open under #633: the tool and modifier page layouts, the fortify variant, the bow-materials section and the smeltery 3D structure page; the resource-pack JSON structure stays gated on the engine port, **#430** (book, medium).
...
- [x] **T50 — Craft and hit sounds** (shipped, issue #481) — all four cues now fire where upstream's do, at upstream's own volumes and pitch spreads: the Tool Station's craft cue (`ContainerToolStation#playCraftSound`, 0.8 / `0.8 + 0.4 * random`) and the Tool Forge's override of it (`ContainerToolForge#playCraftSound`, vanilla `BLOCK_ANVIL_USE` at 0.9 / `0.95 + 0.2 * random` — the one cue that is upstream's actual sound, because upstream uses vanilla's there too), plus both of the frying pan's boings (`FryPan#dealDamage` at 2.0 / flat 1.0 on a landed blow, `FryPan#onPlayerStoppedUsing` at 1.5 / `0.6 + 0.2 * random` on the launch, so a charged launch is two cues). **Audit correction:** the checklist's framing of this as a single ticket conflated the wiring with the asset question — the sound *events* are vanilla stand-ins, the same call #415 (shocking) and #495 (squeaky) made, because upstream's `sounds/Credits.txt` licenses its assets CC-BY/CC0 rather than MIT *and* credits no author for `little_saw.ogg` or `frypan_hit.ogg`, which under CLAUDE.md's Spartan precedent makes shipping them a maintainer decision rather than a parity default. That decision is **#566**; each stand-in is one constant. Bane of Pigs (§3.2's row bundles it here) is a creative-listing easter egg, never in this entry's scope, and stays open (book/tools, medium, M3.5 playtest-fix).
...
- [x] **T57 — World-content content set** (shipped, #488 + #635) — #488 shipped the tree half: the slime sapling (upstream's `BlockSlimeSapling` placement rule, `STAGE`, bone meal and the leaves' 1/25 sapling drop with upstream's own fortune curve), the tree it grows through upstream's `vine == null` canopy branch, the six slime vine blocks (three stages x two colours, upstream's widened "hangs from leaves or from the vine above" support rule and its downward growth), the island generator's `vine != null` canopy branch, and the two `slimevine_*` bowstring materials. #635 shipped the coloured half: the five edible coloured slime balls at upstream's nutrition, saturation and potion effects, congealed slime in all six colours and coloured slime blocks in five, the four-recipe crafting loop per colour plus the mixed-slime-ball fallback behind `matchVanillaSlimeblock`, purple slime's melting recipes, blood slime ball casting, and the knightslime-alloy, magma-mud, blue-crystal and congealed-slime-substitute reverts. (The blue and purple slime fluids and the lake that pours them arrived just ahead of it, with #625.) Two carry-overs, tracked separately: the four **slime drops** (`TinkerCommons:293-297`, upstream's Gadgets-pulse foods) and T22's "one sling instead of five colours" reduction (world, medium, world-content milestone).
...
- [x] **T76 — Creative tab shape** (shipped, #507) — split into General/Tools/Tool Parts/Smeltery tabs, upstream's four populated tabs; the ores upstream files under `tabWorld` ride along in General (Forgeweave has no slime-island world content) and `tabGadgets` has nothing to hold (T56). `listAllToolMaterials` deliberately not added: with it off — upstream's default — the Tools tab holds one entry per tool either way, and Forgeweave's componentless entry is already a recorded deliberate deviation (PR #362) (book/config/tools, low, M6).
