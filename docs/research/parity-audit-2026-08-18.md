...
| Blue slime entity + island spawns | `EntityBlueSlime`, `WorldEvents:29-33` | sky/ender slimes | `ForgeweaveEntities#BLUE_SLIME` + `BlueSlimeRenderer` + `blue_slime_spawns.json` (#451); vanilla slime ball stands in for the blue one | have | Y (SCOPE:245, #451) | low | T57 |
...
- [x] **T20 — Blue slime entity** (shipped, #451) — `forgeweave:blue_slime` ports `EntityBlueSlime` whole: vanilla slime's own body, upstream's greyscale slime texture tinted `0xff67f0f5` on both the body and the gel layer, upstream's one-pool 0-2 drop with its 0-1 looting bonus gated to size 1, the slime-grass-only spawn rule, and the weight-15/2-4 spawn entry plus the `0x47eff5`/`0xacfff4` spawn egg. `slimeyBlue` now spawns it, closing that trait's own stand-in. No entity class of its own: 1.21 answers upstream's four overrides elsewhere (`Slime#remove` splits off `getType()`, a loot-table size condition replaces `getLootTable`, the table names the item `dropItemWithOffset` forced, and `SpawnPlacements` carries `getCanSpawnHere` -- which is where upstream's own 1.20 branch put it). Three recorded reductions: the drop is the vanilla slime ball until T57 ships the coloured ones, the death particle is vanilla's `item_slime` for the same reason, and an island now spawns blue slimes *in addition to* the biome's usual mobs rather than instead of them -- upstream's `WorldEvent.PotentialSpawns` clear does not exist on NeoForge 1.21 and upstream itself dropped the technique in 1.20. The liquid-slime spawn branch waits on T57's fluids (world, high, world-content milestone).
