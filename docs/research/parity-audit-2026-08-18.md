...
- [ ] **T57 — World-content content set** — first slice shipped (#488): the slime sapling (upstream's `BlockSlimeSapling` placement rule, `STAGE`, bone meal and the leaves' 1/25 sapling drop with upstream's own fortune curve), the tree it grows through upstream's `vine == null` canopy branch, the six slime vine blocks (three stages x two colours, upstream's widened "hangs from leaves or from the vine above" support rule and its downward growth), the island generator's `vine != null` canopy branch, and the two `slimevine_*` bowstring materials. Still open, all of it downstream of the coloured slime balls: congealed and coloured slime blocks in every colour, the coloured slime balls themselves, blue/purple slime fluids, blood slime ball casting, the slime lake, and then the knightslime-alloy, magma-mud and blue-crystal reverts (world, medium, world-content milestone).
