...
3. **Commitment**: at M6, trait and modifier *definitions* become datapack-creatable via a parameterized behavior library (~10–15 generic Java behaviors — `potion_effect_on_hit`, `bonus_damage_vs`, `speed_scales_with_durability`, … — composed with parameters in JSON). KubeJS bindings are evaluated at M8.
   ...
   - **Delivered at M8 by #973 (modifiers), closing this item.** Maintainer decision 2026-09-18: the `forgeweave:modifier_definition` datapack registry ships shaped exactly like `trait_definition`, so a pack author learns one idiom. A `behavior` field dispatches over one codec per class in `ModifierLibrary` (16 behaviors, every one extracted from a shipped Java modifier, listed in docs/SCOPE.md's M8 section), with three shared optional fields carrying what every modifier answers regardless of what it does: `units_per_level`, `slots` and `tools`. Synced with the codec as network codec, existence-gated by `neoforge:conditions`, built-in ids winning any collision, and behind the `compat.modifierDefinitions` toggle (D-M8-5), which is read at lookup because a SERVER config does not exist when datapack registries load. Serialization does not move: a tool still stores `id + level` (item 2), so every save and every save-compat fixture stays valid and a tool whose pack is gone keeps the entry inertly. Modifiers keep their application recipes (item 1) unchanged -- this adds a second axis rather than replacing the first. The **KubeJS modifier builder was deliberately not built**: the maintainer's decision is to add a `ForgeweaveEvents.modifiers` sibling only when the first pack author asks for it, since a binding with no user is a maintenance cost with no return. The behaviors no parameter set can express stay Java, which `ModifierBehaviors`' javadoc lists (haste's and sharpness's diminishing curves, the event-driven modifiers, the generated per-material ids, and the compat-owned ones).
