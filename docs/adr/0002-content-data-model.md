...
- **Amended at M6 (2026-09-02, #832)**: the trait half of that commitment is delivered. "Trait behavior is Java code" now reads: trait *behaviour classes* are Java; a trait *definition* may be Java (the shipped roster), a `trait_definition` datapack entry composing one behaviour class with parameters, or a KubeJS startup-script `ScriptTrait` for logic parameters cannot express. Material JSON still only names an id. Modifier definitions stay code until the M8 decision recorded in ADR-0004 item 3.
