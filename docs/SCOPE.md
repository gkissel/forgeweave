...
| `warmace` | `vanilla_mace` | `fist` | `WarmaceItem` delegates its combat hooks straight to vanilla's `MaceItem`, so Better Combat's preset for that exact vanilla item is an exact behavioral match; Epic Fight has no mace type, so its blunt-impact `fist` stands in |
| `battlesign` | `staff` | `shield` | a flat implement whose innate blocks and reflects; Epic Fight's shield type is a direct thematic match, Better Combat has none so its closest pole-mounted guard implement stands in |
| `frying_pan` | `hammer` | `fist` | a single blunt head on a handle, both mods' blunt-impact presets |
